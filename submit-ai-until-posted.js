...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a.startsWith('--agent=')) opts.agent = a.split('=')[1];
...
(async function main() {
  ...
  // --agent=human 时把 buildSets 里的 --submit-ai 替换为 --submit-human
  if (opts.agent === 'human') {
    opts.buildSets = opts.buildSets.map(bs => bs.map(a => a === '--submit-ai' ? '--submit-human' : a));
  }
  const version = formalVersion();
  console.log(`目标版本：${version}`);
  console.log(`最多尝试 ${opts.maxAttempts} 次，每次命中后轮询${opts.agent}榜 ${opts.verifyPolls} 次。`);
  ...
  console.log(`❌ 在最大尝试次数内仍未打出可提交的${opts.agent === 'human' ? '人类' : 'AI'}成绩。`);
  process.exit(1);
})().catch(err => {
  console.error(err);
  process.exit(1);
