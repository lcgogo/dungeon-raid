...
| 🧌 兽人 | 斗士 → 血狂 Blood Frenzy | 吸血/嗜血回血超出生命上限时，把溢出的部分全部增为永久生命上限 |
...
| ⛄ 雪人 Snowman | 仅炸弹 | 现身就**随机冰封你的主动**——按 Boss 档位冻 1~3 个（职业主动/💊治疗/💥炸弹 里随机），4 回合内不可用（❄️ 标记）；之后**每次倒计时归零再次冰封**。**本身不攻击、不掉你血**——趁解冻空档炸掉它（再冻间隔比冻结时长长，总有可炸窗口）|
...
- 🏆 **云排行榜 + 百分位（仅正式版 `dungeonraid.win`）**：死亡后显示你在**总榜**和**本种族榜**的名次与百分位。按**坚持回合**排名；榜单**合并最近 3 个版本**展示（避免频繁补丁把榜单切碎），**只展示已重放验证的成绩**，前列每小时自动重放校验防作弊。开发版不参与排名。可在开始页设置**排行榜名字**（最长 12 汉字，🎲 随机起名，本机记住），榜单每行显示「种族头像 + 名字 + 回合/等级」；**点任意一行（ⓘ）弹框**看本局回合/等级、职业线与技能、是否破关，以及 ▶回放 / 🔗分享。
...
| 🧌 Orc | Fighter → Blood Frenzy | When Lifesteal/Bloodthirst healing exceeds max HP, the full overflow becomes permanent max HP |
...
| ⛄ Snowman | Bomb only | On arrival it **freezes random actives** — 1 at Lv1, 2 at Lv2, 3 at Lv3 (class active / 💊Heal / 💥Bomb), unusable for 4 turns (❄️ marker); then **re-freezes every time its countdown hits 0**. It deals **NO damage itself** — bomb it during the thaw (re-freeze interval > freeze duration, so a window always opens) |
...
- 🏆 **Cloud leaderboard + percentile (Release build `dungeonraid.win` only)**: after death, see your rank and percentile on the **overall** and **per-race** boards. Ranked by **turns survived**; the board **merges the latest 3 builds** (so frequent patches don't fragment it), **shows verified scores only**, and the top is re-verified hourly to block cheats. The Dev build does not participate in ranking. Set a **leaderboard name** on the start screen (≤12 CJK, 🎲 to randomize, remembered locally); each row shows "race avatar + name + turns/level"; **tap any row (ⓘ)** for a popup with that run's turns/level, class line & perks, clear status, plus ▶Replay / 🔗Share.
