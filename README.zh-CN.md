...
- 🏅 **人类榜 / AI 榜**：成绩按 `agent`(human / ai)分流、互不混排。游戏内「🏆 排行榜」面板可在 **人类 / AI**、**闯关榜 / 破关榜** 间切换，并**按种族筛选**（全部 / 人族 / 精灵 / 矮人 / 兽人 / 亡灵）查看 Top 10。真人对局上报人类榜，AI/机器人自报 `ai` 进 AI 榜（后端 `/top?agent=&race=`、`/clearboard?agent=&race=`，私有改判接口 `/classify`）。GitHub Actions 现在会定时给人类榜前 200 条录像打 AI 可疑度分，**凡是 `score >= 25` 的条目都会自动改判到 AI 榜**；这里的 `Low / Medium / High / Very High` 仍只是可疑度标签，不等同于最终是否进 AI 榜。也就是说，新的策略会把一部分 `Medium` / `High` 风险录像也直接移到 AI 榜。AI/无头提交流程也和真人一样必须保留服务端 `/seed` 下发的 `rec.token`，否则会被 worker 拒绝为离线对局。写入端点按 IP 限流防刷。
