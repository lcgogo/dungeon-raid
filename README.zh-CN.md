...
- 🏆 **云排行榜 + 百分位（仅正式版 `dungeonraid.win`）**：死亡后显示你在**总榜**和**本种族榜**的名次与百分位。按**坚持回合**排名；榜单按 **`v主.次` 小版本桶** 合并最近 3 桶展示（避免频繁补丁把榜单切碎），**只展示已重放验证的成绩**。verify 仍会按每局自己的精确 `rec.ver` 选 `engines/<version>.html` 快照重放；因此凡是会影响 verify / replay 兼容边界的改动，都应该升 **次版本**，而不是继续挤在同一个 patch 序列里。开局时服务端会随 seed 下发按固定回退链解析后的上传门槛快照：**同种族 + 当前 minor → 同种族 + 最近 3 个 minor → 全种族 + 当前 minor → 全种族 + 最近 3 个 minor**。也就是说，如果当前种族 / 当前 minor 样本不足，就会逐层回退到更稳定的范围，而不是硬用一个过小样本桶。计算门槛时仍排除 `turns >= 510` 的近终局成绩，并把最终 upload gate 限制在 350 回合内。当前若成绩**低于解析后的上传门槛**，结算页只显示“约分位”、不再正式上传进验证链，减少无意义验证。顶尖成绩只有在 render 验证器已切到同一 release 版本时才会触发即时验证，否则交给轮询 / GitHub cron 兜底。开发版不参与排名。可在开始页设置**排行榜名字**（最长 12 汉字，🎲 随机起名，本机记住），榜单每行显示「种族头像 + 名字 + 回合/等级」；**点任意一行（ⓘ）弹框**看本局回合/等级、职业线与技能、是否破关，以及 ▶回放 / 🔗分享。
