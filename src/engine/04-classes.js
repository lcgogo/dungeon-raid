...
const TIER1={
  ...
  miser:{n:['守财奴','Miser'], quip:['钱躺一会儿，利滚利。','Let the gold nap — it wakes up bigger.'], skill:{name:['囤金','Hoard'], short:['投入手头金币，4回合后1.2倍返还','Invest current gold; 1.2× in 4 turns'], desc:['把当前手头金币全部投入囤金，4 回合后按剩余投入金币的 1.2 倍返还。期间新获得的金币照常进账，也可以继续使用商店和其它技能；若已有「钱能买命」，受伤时只消耗手头金币，不会动囤金本金。没有金币或正在囤金时无效、不进冷却。','Invest all current gold into Hoard; after 4 turns, the remaining invested gold is paid back at 1.2×. New gold earned during Hoard is banked normally, and shops / other skills remain usable. With Money Buys Life, damage always spends current wallet gold first and never touches invested Hoard gold. No effect — and no cooldown — with no gold or while already hoarding.'], cd:5, f:p=>{ if(p.goldLock>0) return false; const stake=p.gold|0; if(stake<=0) return false; p.gold=0; p.goldFrozen=stake; const n=4; p.goldLock=n; log(tr(`🔒 囤金：投入 ${stake} 金币，${n} 回合后按 1.2 倍返还`,`🔒 Hoard: invested ${stake} gold; 1.2× payout in ${n} turns`), 'buff'); return true; }}},
  ...
  necromancer:{n:['死灵','Necromancer'], quip:['你的魂，我借来续命。','Your soul keeps me going. Thanks.'], skill:{name:['吸魂大法','Soul Drain'], short:['随机吸 2 个目标的生命补自己','Drain 2 random targets to heal'], desc:['随机选 2 个目标下手——普通怪或 Boss 都行，<b>含 {W} 免疫的特殊 Boss</b>（幽灵/小丑/污染怪/雪人），仅终焉之主除外。<br>对每个目标，吸取「你当前生命上限」与「它当前血量」里<b>较小</b>的那个：所以<b>吸力随你的生命上限成长</b>——上限越高，单发吸得越狠、越容易一发吸死（吸取量 ≥ 它血量即死亡）。<br>吸到的生命总量给你回血，<b>封顶在生命上限</b>（活死人「一切治疗减半」对这部分也生效）。<br>协同：① 学了「溅射」，被吸死目标多出的吸力（你的上限 − 它血量）会溅到剩余的一个敌人；② 击杀同样会触发「汲取生命」（前提是你还有血量空间）。<br>打法：应急回血 + 专治 {W} 免疫 Boss；越堆生命上限（巨力等）越强。','Drains 2 random targets — regular monsters or bosses alike, <b>including {W}-immune special bosses</b> (Ghost/Clown/Corruptor/Snowman); only the Overlord is exempt.<br>From each it takes the <b>lesser</b> of your current max HP and the target’s current HP — so its power <b>scales with YOUR max HP</b>: the higher your cap, the harder each drain hits and the more likely it kills outright (drain ≥ its HP = dead).<br>The total drained heals you, <b>capped at your max HP</b> (Undead’s halved-healing applies here too).<br>Synergy: (1) with Splash, a lethal drain’s leftover power (your max HP − its HP) splashes to one more foe; (2) kills also trigger Drain Life (if you have HP to spare).<br>Play: emergency healing + a hard counter to {W}-immune bosses; scales with max-HP builds (Titan, etc.).'], cd:5, f:p=>{
    ...
    let healed=0, kills=0, normalKills=0; const pick=foes.slice(0,2); const splashes=[];
    for(const [r,c] of pick){ const t=grid[r][c]; const before=t.hp; const drain=Math.min(p.maxHp, before); t.hp-=drain; healed+=drain; statueReflect(t, drain);   // 石像：吸魂的伤害也会反弹
      if(t.hp<=0){ const isB=t.type==='boss'; if(isB){if(defeatBoss(t,r,c)==='split')continue;}else{grid[r][c]=null;normalKills++;addXp(p,3+(p.killXp||0));gainGold(1);} kills++;
    ...
    let splashK=0; for(const over of splashes){ const sr=splashHit(over); splashK+=sr.kills; normalKills+=sr.normalKills; }   // 溅射独立结算（含剑免疫 Boss，排除终焉）
    kills+=splashK;
    const multiGold=multiKillGold(normalKills), h=gainHeal(healed); const lh=lifestealHeal(kills); applyGravity(); syncPositions(false);
    log(tr(`🪄 吸魂大法：吸取 ${pick.length} 个目标 ${healed} 生命，回血 ${h}`+(kills?`，击杀 ${kills} 只`:'')+(multiGold?`，多杀奖励 +${multiGold} 金`:'')+(lh?`，吸血 +${lh}`:'')+(splashes.length?`，溅射 ${splashes.length} 次`:''),`🪄 Soul Drain: drained ${healed} from ${pick.length}, +${h} HP`+(kills?`, ${kills} killed`:'')+(multiGold?`, multi-kill +${multiGold} gold`:'')+(lh?`, +${lh} lifesteal`:'')+(splashes.length?`, ${splashes.length} splash`:'')), 'heal'); return true; }}},
  ...
  lich:{n:['巫妖','Lich'], quip:['感受死亡的寒冷吧。','Feel the cold of death.'], skill:{name:['冰封球','Frost Orb'], short:['全场固定伤害+全体减速','Flat damage + slow all foes'], desc:['对场上所有怪物/Boss（终焉之主除外）造成一次当前固定伤害，并让它们的当前攻击倒计时 +1。','Deals current flat damage to all foes and delays their next attack.'], cd:5, f:p=>{ addBossFx(Math.floor(ROWS/2), Math.floor(COLS/2), false, '#7fd3ff', true); let hit=0, kills=0, normalKills=0, total=0, lh=0; const dmg=frostOrbDamage(), actors=[]; for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++){const t=grid[r][c];if(t&&!t.finale&&(t.type==='enemy'||t.type==='boss'))actors.push({r,c,t});} for(const {r,c,t} of actors){if(grid[r][c]!==t)continue; const before=t.hp; t.hp-=dmg; t.cd+=1; t.frostTurns=3; hit++; total+=Math.min(dmg,before); if(t.hp<=0){if(t.type==='boss'){if(defeatBoss(t,r,c)==='split')continue;}else{grid[r][c]=null;normalKills++;addXp(player,3+(player.killXp||0));gainGold(1);}kills++;}} const multiGold=multiKillGold(normalKills); if(kills)lh=lifestealHeal(kills); applyGravity(); syncPositions(false); log(tr(`🧊 冰封球：全场 ${hit} 个目标共掉 ${total}`+(kills?`，击杀 ${kills}`:'')+(multiGold?`，多杀奖励 +${multiGold} 金`:'')+(lh?`，吸血 +${lh}`:''),`🧊 Frost Orb: ${hit} targets lose ${total}`+(kills?`, ${kills} killed`:'')+(multiGold?`, multi-kill +${multiGold} gold`:'')+(lh?`, +${lh} lifesteal`:'')), 'buff'); return hit||kills||lh; }}},
  butcher:{n:['屠夫','Butcher'], quip:['钩子一甩，肉就乖乖到底。','One swing of the hook, and the meat comes to me.'], skill:{name:['钩子','Hook'], short:['拉到底排；离底越远伤害越高','Drag down; farther from bottom = more damage'], desc:['一钩把棋盘上所有的怪和 Boss（含{W}免疫，终焉之主除外）顺着各自所在列拉到尽可能底部，其余棋子上浮；随后按【离底线的距离】造成伤害：最底排吃 1× 固定伤害、倒数第二排吃 2×，依此类推。场上没有可拉的敌人时无效、不进冷却。','Hooks every monster & boss (incl. {W}-immune; not the Overlord) down to the bottom of its column while other tiles float up; then deals damage by【distance from the bottom】— bottom row takes 1× flat damage, second-from-bottom 2×, and so on. No effect — and no cooldown — when there is nothing to pull.'], cd:5, f:p=>{
    let moved=0, any=false, dmg=0, kills=0, normalKills=0; const isMon=t=>t&&(t.type==='enemy'||t.type==='boss')&&!t.finale; const hookBase=Math.max(1, p.swordFlat||0); const lines=[];
    ...
    for(let r=ROWS-1;r>=0;r--)for(let c=0;c<COLS;c++){ const t=grid[r][c]; if(!isMon(t)) continue;
      ...
      if(t.hp<=0){ const isB=t.type==='boss'; if(isB&&defeatBoss(t,r,c)==='split')continue; if(!isB){grid[r][c]=null;normalKills++;addXp(player,3+(player.killXp||0));gainGold(1);if(player.rotflesh)player.maxHp++;} kills++; } }
    const multiGold=multiKillGold(normalKills), lh=lifestealHeal(kills); applyGravity(); syncPositions(false);   // 满盘时 applyGravity 为空操作、不消耗 RNG；纯重排，重放确定
    log(tr(`🪝 钩子：把全场怪/Boss 拉到底排，造成 ${dmg} 伤害`+(kills?`，击杀 ${kills}`:'')+(multiGold?`，多杀奖励 +${multiGold} 金`:'')+(lh?`，吸血 +${lh}`:''),`🪝 Hook: dragged all foes down for ${dmg} damage`+(kills?`, ${kills} killed`:'')+(multiGold?`, multi-kill +${multiGold} gold`:'')+(lh?`, +${lh} lifesteal`:'')), moved?'buff':''); return true; }}},
  azuredragon:{n:['青龙','Azure Dragon'], quip:['接受来自远古的绝对威压吧！','Accept the absolute pressure of the ancients!'], skill:{name:['龙吟','Dragon Roar'], short:['全场造成等级伤害','Deal level damage to all foes'], desc:['对全场所有怪物（包括普攻免疫 Boss）造成等同当前等级的伤害。','Deal damage equal to your current level to every foe, including normal-attack-immune bosses.'], cd:5, f:p=>{ addBossFx(Math.floor(ROWS/2), Math.floor(COLS/2), false, '#f5b041', true); const r=dealDamage(p.level,null,true); applyGravity(); syncPositions(false); log(tr(`🐉 龙吟：全场 ${r.dmg} 伤害`+ (r.kills?`，击杀 ${r.kills}`:'')+xpLog(r.xp),`🐉 Dragon Roar: ${r.dmg} damage to all foes`+(r.kills?`, ${r.kills} killed`:'')+xpLog(r.xp)), 'buff'); return true; }}},
