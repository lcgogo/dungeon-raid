...
### 平衡
- **升级「强化体魄」不再回满血，改为回等量血**：原本每次选「强化体魄」都**立即回满**（白嫖一次满血复原），后期相当于无限免费全恢复，太强、不合理。现改为**只回复本次提升的生命上限那么多**（人族 +6 上限 / 回 6；兽人 ×2 = +12/回12；精灵 ×0.5 = +3/回3）——升级仍给一小笔回血，但不再是满血重置。回测（每档 40 局）显示各档生存中位数下降约 7–14%，整体仍能正常推进、够到里程碑。（确定性，重放/榜单按版本分桶不受影响。）
