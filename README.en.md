...
The current Dev build is `v1.74.1`; race cards now support long-press details for their traits and weaknesses, and the Statue boss has 2× the same-level base HP. Replay settlement screens now show the replay's actual final level, gold, and turns instead of the state from before replay started. Turn-50 advancement details now use a stable label/content layout so long effects wrap consistently. Turn-50, turn-100, and turn-200 advancement prompts explicitly tell players to long-press a choice for details, while turn-350 Crossover Skill choices have an `ⓘ` details button. Long combat logs now wrap automatically. Frozen Heal/Bomb actives no longer auto-cast during a Snowman freeze. Multi-kills of normal enemies still grant bonus gold equal to the number killed. The DEV start card shows the version directly. The installed PWA opens the home page first, where you can choose Release or Dev. Source lives under `src/` and the self-contained HTML is regenerated with `node build/build.js`.
...
- **Gold shop**: 💊 Heal (restore 10 HP, **+2 healing and +1 gold cost per use**; **unavailable at full HP** to avoid waste) / 💥 Bomb (all foes' HP −5). Each item has its own cooldown.
...
- 📡 **Offline navigation fallback**: if the network is unavailable and a navigation URL is not cached exactly, the Service Worker falls back to the cached game shell instead of handing iOS Safari an empty response.
