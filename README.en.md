...
The current Dev build is `v1.71.9`; the DEV start card shows the version directly so you can confirm whether a local or deployed page is up to date. The installed PWA now opens the home page first, where you can choose Release or Dev. Source lives under `src/` and the self-contained HTML is regenerated with `node build/build.js`.
