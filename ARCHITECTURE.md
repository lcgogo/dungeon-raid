...
没有攻击力、不会直接让玩家掉生命的 Boss 默认武器免疫；会直接攻击玩家的 Boss 默认可被武器攻击。幽灵是唯一的明确例外：它会攻击玩家，但仍保持武器免疫。所有武器免疫 Boss 的血量都使用同级普通怪血量；其攻击力和倒计时公式保持各自原有规则。炸弹、毒心、吸魂等非武器伤害仍可按各 Boss 机制命中武器免疫目标。
